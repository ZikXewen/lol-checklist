...
const stored = localStorage.getItem('champions-list')
let list: Champion[]
if (!stored) {
  //prettier-ignore
  ...
  list = championsList.map((name) => ({
    name,
    checked: false,
  }))
  localStorage.setItem('champions-list', JSON.stringify(list))
} else list = JSON.parse(stored)

const root = document.getElementById('root')
list.forEach(({ name, checked }) => {
  const newEl = document.createElement('button')
  newEl.innerText = name
  if (checked) newEl.classList.add('checked')
  newEl.addEventListener('click', (e) => {
    const target = e.target as HTMLButtonElement | null
    const id = list.findIndex(({ name }) => name === target?.innerText)
